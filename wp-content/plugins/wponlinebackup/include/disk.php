...
HEADER;
	...
	/*public*/ function Write( $data, $length = null )
	{
		...
		while ( true ) {
			...
			if ( $written == 0 )
				return $this->Partial_Write( 'write', $length - $todo_length, $length );
	...
	/*public*/ function Rewrite( $offset, $data, $length = null )
	{
		...
		if ( $offset >= $this->offset ) {
			...
			while ( true ) {
				...
				// If we wrote nothing, fail
				if ( $written == 0 )
					return $this->Partial_Write( 'rewrite', $length - $todo_length, $length );
			...
			while ( true ) {
				...
				// If we wrote nothing, fail
				if ( $written == 0 )
					return $this->Partial_Write( 'rewrite', $this_length - $todo_length, $this_length, 'patch' );
	...
	/*public*/ function Do_Reconstruct()
	{
		...
		while ( true ) {
			...
			// If we wrote nothing, fail
			if ( $written == 0 ) {
				$ret = $this->Partial_Write( 'write', $length - $todo_length, $length, 'rc' );
				@fclose( $this->rc_file );
				return $ret;
			}
	...
	/*private*/ function Write_Rejection_Header( $file, $type = '' )
	{
		...
		while ( true ) {
			...
			// If we wrote nothing, fail
			if ( $written == 0 )
				return $this->Partial_Write( 'write header', $length - $todo_length, $length, $type );
