...
class WPOnlineBackup_Backup_Tables
{
	...
	/*public*/ function Backup( & $bootstrap, & $stream, & $progress, & $job )
	{
		...
		if ( $job['table'] === true ) {
			...
			$header =
					'-- ' . _x( 'Online Backup for WordPress', 'DB backup header', 'wponlinebackup' ) . PHP_EOL .
					'-- ' . sprintf( _x( 'Version %s', 'DB backup header', 'wponlinebackup' ), WPONLINEBACKUP_VERSION ) . PHP_EOL .
					'-- ' . _x( 'http://www.backup-technology.com/free-wordpress-backup/', 'DB backup header', 'wponlinebackup' ) . PHP_EOL .
					'-- ' . PHP_EOL .
					'-- ' . sprintf( _x( 'Blog: %s', 'DB backup header', 'wponlinebackup' ), get_bloginfo( 'wpurl' ) ) . PHP_EOL .
					'-- ' . sprintf( _x( 'Dump Time: %s', 'DB backup header', 'wponlinebackup' ), date( $date_format ) ) . PHP_EOL .
					PHP_EOL .
	...
	/*public*/ function Backup_Table()
	{
		...
		while ( $this->job['progress'] != 100 ) {
			...
			while ( false !== ( $next_row = $this->Fetch_Row( $result ) ) ) {
				...
				$insert .=
					( $row_count == 1 ?
						'INSERT INTO `' . $this->job['table'] . '` (' . $fields . ') VALUES' . PHP_EOL :
						',' . PHP_EOL
					) .
					'(' . implode( ',', $values ) . ')';
